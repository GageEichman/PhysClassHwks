...
#THIS ONE MAKES THE X VS Y GRAPHS
...
def get_accel(pos, Mass, G, Softening, N, Charge, K):
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            dist = (dx ** 2 + dy ** 2 + dz ** 2 + Softening ** 2) ** (
                        -3 / 2)  # just multiply by dist, includes 1/ in the negative sign
...
def main():
    ...
    pos_save = np.zeros(
        (N, 3, Nt + 1))  # like a 3d brick where each slice of the brick is a frame/ all positions at time t
    ...
    for i in range(Nt):
        ...
        if plotRealTime or (i == Nt - 1):
            ...
            plt.pause(
                0.01)  # this loop basically just updates the graph every 0.01 seconds, creating a new graph. however the pos save allows you to update the next frame with the position of the particle in the old frame
